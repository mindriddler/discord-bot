...
class DiscordBotLogger:
    ...
    def __init__(
        ...
        log_rotation: int | str | datetime.timedelta |
        datetime.time = logger_config["log_rotation"],
    ...
    def get_dm_logger_for_user(self, user_id: str) -> loguru.Logger:
        ...
        if not hasattr(loguru.logger.__class__, custom_level_name):
            self.add_custom_level(custom_level_name,
                                  level=5,
                                  color="<magenta>",
                                  icon="-")
            loguru.logger.__class__.user_id = partialmethod(
                loguru.logger.__class__.log, custom_level_name)
