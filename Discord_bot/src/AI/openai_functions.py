...
class OpenAIFunctions:
    ...
    async def handle_dm(self, message, user_message, bot_response, bot_user,
                        user_id):
        if user_id not in self.dm_loggers:
            ...
            self.dm_loggers[user_id].user_id(
                f"{bot_user} >> {user_id}: Hello {user_id}, {DEFAULT_DM_MESSAGE}"
            )
            ...
            self.dm_loggers[user_id].user_id(
                f"{user_id} >> {bot_user}: {user_message}")
        else:
            if isinstance(message, discord.Interaction):
                self.dm_loggers[user_id].user_id(
                    f"{bot_user} >> {user_id}: {bot_response}")
